...
class CalibratedClassifierCV(ClassifierMixin,
    ...
    def fit(self, X, y, sample_weight=None, **fit_kargs):
            ...
            if self.ensemble:
                ...
                self.calibrated_classifiers_ = parallel(
                    delayed(_fit_classifier_calibrator_pair)(
                        ...
                        supports_sw=supports_sw, sample_weight=sample_weight, **fit_kargs)
                ...
                pred_method = partial(
                    ...
                    cv=cv, method=method_name, n_jobs=self.n_jobs, fit_params=fit_kargs
                )
                predictions = _compute_predictions(pred_method, X, n_classes)

                if sample_weight is not None and supports_sw:
                    this_estimator.fit(X, y, sample_weight, **fit_kargs)
                else:
                    this_estimator.fit(X, y, **fit_kargs)
                calibrated_classifier = _fit_calibrator(
    ...
    def score(self, X, y, sample_weight=None):
        if len(self.classes_) > 2:
            score = roc_auc_score(y, self.predict_proba(X), sample_weight=sample_weight)
        else:
            score = roc_auc_score(y, self.predict_proba(X)[:, 1], sample_weight=sample_weight)
        return score
...
def _fit_classifier_calibrator_pair(estimator, X, y, train, test, supports_sw,
                                    method, classes, sample_weight=None, **fit_kargs):
    ...
    if sample_weight is not None and supports_sw:
        estimator.fit(X[train], y[train],
                      sample_weight=sample_weight[train], **fit_kargs)
    else:
        estimator.fit(X[train], y[train], **fit_kargs)
